...
public class write extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String title = request.getParameter("title") + ".txt";
        FileWriter writer;

        String tea_ID = request.getParameter("teaid");//获取文件目录
        String cou_ID = request.getParameter("couid");
        String path = request.getServletContext().getRealPath("/upload/" + tea_ID + "/" + cou_ID + "/source");//获取路径

        String filePath = path + "/" + title;//得到文件路径和名称组合
